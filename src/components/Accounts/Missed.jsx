...
import { FaArrowLeft, FaExclamationCircle, FaDownload, FaSearch, FaFilter, FaCalendar, FaReceipt, FaUser, FaMoneyBillWave, FaCheckCircle } from 'react-icons/fa';
...
const Missed = () => {
  ...
  const fetchMissedPayments = async () => {
    try {
      ...
      console.log('Fetching missed payments with params:', params.toString());
      const response = await fetch(`http://localhost:5001/api/accounts/all?${params}`);
      const result = await response.json();
      console.log('Missed payments API response:', result);
      ...
      if (result.success) {
        ...
        setError(result.message || 'Failed to fetch missed payments');
      }
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen p-4 md:p-6 flex items-center justify-center bg-secondary">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto"></div>
          <p className="mt-4 text-muted-foreground">Loading missed payments...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-foreground">Missed Payments</h1>
              <p className="opacity-70 mt-1 text-muted-foreground">Overdue payments requiring attention</p>
              {universityInfo && (
        ...
        <div className="rounded-lg shadow-lg p-6 bg-card border border-border">
          <div className="text-center">
            <div className="mb-4">
              <FaExclamationCircle className="w-12 h-12 text-foreground mx-auto mb-3" />
              <h2 className="text-2xl font-bold text-foreground mb-2">Total Missed Amount</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-secondary rounded-lg p-4">
                ...
                <p className="text-muted-foreground text-sm">Students with Overdue</p>
              ...
              <div className="bg-secondary rounded-lg p-4">
                ...
                <p className="text-muted-foreground text-sm">Average Overdue</p>
            ...
            <table className="min-w-full">
              <thead className="bg-secondary">
                <tr className="border-b border-border">
                  ...
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-muted-foreground">Days Overdue</th>
              ...
              <tbody className="divide-y divide-gray-700">
                {paginatedPayments.map((payment) => {
                  const daysOverdue = Math.floor((new Date() - new Date(payment.emi_duedate)) / (1000 * 60 * 60 * 24));
                  return (
                    <tr key={payment._id} className="hover:bg-secondary transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-foreground font-medium">{daysOverdue} days</td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <FaExclamationCircle className="w-4 h-4 text-foreground" />
        ...
        {payments.length === 0 && !loading && (
          <div className="text-center py-12 rounded-xl shadow-lg bg-card border border-border">
            <FaExclamationCircle className="w-16 h-16 mx-auto text-gray-600 mb-4" />
            <p className="text-xl text-muted-foreground">No missed payments found</p>
            <p className="text-sm text-gray-500 mt-2">Great! All payments are up to date</p>
          </div>
        )}
